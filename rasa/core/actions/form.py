...
from rasa.core.events import SlotSet, Form, Event
...
class FormAction(Action):
    ...
    @staticmethod
    def required_slots(tracker: "DialogueStateTracker") -> List[Text]:
    ...
    def entity_is_desired(
        self, requested_slot_mapping: Dict[Text, Any], slot: Text, tracker: "DialogueStateTracker"
    ) -> bool:
        ...
        for slot, value in list(slot_dict.items()):
            ...
            if inspect.iscoroutinefunction(validate_func):
                validation_output = await validate_func(
                    value, output_channel, nlg, tracker, domain
                )
            else:
                validation_output = validate_func(value, output_channel, nlg, tracker, domain)
    ...
    async def validate(
        ...
        slot_to_fill = tracker.get_slot(REQUESTED_SLOT)
        if slot_to_fill:
            slot_values.update(self.extract_requested_slot(output_channel, nlg, tracker, domain))
        ...
        for slot in self.required_slots(tracker):
            if self._should_request_slot(tracker, slot):
                ...
                draft = await nlg.generate(
                    f"utter_ask_{slot}", tracker, output_channel.name(), **tracker.slots
                )
                return [create_bot_utterance(draft), SlotSet(REQUESTED_SLOT, slot)]
    ...
    async def _validate_if_required(
        ...
        if tracker.latest_action_name == "action_listen" and tracker.active_form.get(
            ...
            if inspect.iscoroutinefunction(self.validate):
                ...
                return self.validate(output_channel, nlg, tracker, domain)
        else:
    ...
    async def run(
        ...
        # validate user input
        events.extend(await self._validate_if_required(output_channel, nlg, tracker, domain))
        ...
        if Form(None) not in events:
            ...
            for event in events:
                ...
                if event_dict["event"] == "slot":
                    temp_tracker.slots[event_dict["name"]] = event_dict["value"]
            ...
            if next_slot_events is not None:
                ...
                if inspect.iscoroutinefunction(self.submit):
                ...
                if inspect.iscoroutinefunction(self.deactivate):
